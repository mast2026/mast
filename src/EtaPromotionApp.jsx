...
import mastLogo from "./assets/mast-wordmark.png";
...
import mastHeroMegaphone from "./assets/mast-hero-megaphone-cropped.png";

var ADMIN_CODE = import.meta.env.VITE_ADMIN_CODE || "mast2026!";
var MISSION_BUCKET = "missions";
var assets = {
  hero: { megaphone: mastHeroMegaphone },
...
function MemberApp(props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7FAFF", fontFamily: FONT, paddingBottom: 84 }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 84, background: "#fff", borderTop: "1px solid #E5EAF2", display: "flex", zIndex: 30, maxWidth: 480, margin: "0 auto", boxShadow: "0 -8px 24px rgba(33,64,120,0.04)" }}>
        {tabs.map(function(t) {
...
function MemberHome(props) {
  ...
  return (
    <div style={{ padding: "30px 12px 18px", background: "#F7FAFF", minHeight: "calc(100vh - 84px)" }}>
      <div style={{ padding: "0 12px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <img src={mastLogo} alt="MAST" style={{ width: 102, height: "auto", display: "block" }} />
          <button aria-label="알림" style={{ border: "none", background: "transparent", padding: 4, color: "#1269F3", display: "flex", cursor: "pointer" }}>
            <IconBell color="#1269F3" />
          </button>
        </div>
        <div style={{ fontSize: 16, lineHeight: "22px", color: "#71819D", fontWeight: 700, marginBottom: 2 }}>안녕하세요,</div>
        <div style={{ fontSize: 22, lineHeight: "30px", fontWeight: 900, color: "#08235E", marginBottom: 20 }}>
          {session.member.name}님 <span style={{ fontSize: 22 }}>👋</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={homeHeroCard()}>
          ...
          <div style={{ position: "absolute", zIndex: 3, left: 174, top: 58, right: 12, textAlign: "center" }}>
            <div style={{ fontSize: 27, lineHeight: "31px", fontWeight: 900, color: "#071C59", letterSpacing: 0 }}>{isAssignee ? "오늘" : "오늘은"}</div>
            <div style={{ fontSize: isAssignee ? 28 : 24, lineHeight: isAssignee ? "34px" : "29px", fontWeight: 900, color: isAssignee ? "#2F78F6" : "#7C88A0", letterSpacing: 0, whiteSpace: "normal", wordBreak: "keep-all" }}>{isAssignee ? "대상자입니다" : "대상자가 아니에요"}</div>
            ...
            <div style={homeCard({ minHeight: 96, padding: 12, display: "flex", flexDirection: "column", justifyContent: "space-between" })}>
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, lineHeight: "18px", color: "#08235E", marginTop: 2, fontWeight: 700 }}>{myAssignment && myAssignment.submitted_at ? "오늘 " + fmtTime(myAssignment.submitted_at) + "에 제출했어요" : "아직 제출하지 않았어요"}</div>
                </div>
              </div>
...
function AdminApp(props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7FAFF", fontFamily: FONT, display: isMobile ? "block" : "flex" }}>
      {isMobile && (
        <div style={{ position: "sticky", top: 0, zIndex: 20, background: "rgba(247,250,255,0.98)", borderBottom: "1px solid #E5EAF2", padding: "10px 10px 8px", boxShadow: "0 8px 24px rgba(33,64,120,0.05)", boxSizing: "border-box", width: "100%", overflow: "visible" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
              <img src={mastLogo} alt="MAST" style={{ width: 86, height: "auto", display: "block" }} />
            ...
            <button onClick={props.onLogout} style={btnSmall({ width: "auto", background: "#fff", color: SUB, border: "1px solid #E5EAF2", borderRadius: 14, whiteSpace: "nowrap", padding: "7px 11px", fontSize: 11 })}>로그아웃</button>
          </div>
            ...
            {mobileMainNav.map(function(it) {
              ...
              return (
                <button key={it.key} onClick={function() { setTab(it.key); setMoreOpen(false); }}
                  style={{ minWidth: 0, border: "none", borderRadius: 12, background: active ? "#0869F4" : "transparent", cursor: "pointer", fontFamily: FONT, display: "flex", alignItems: "center", justifyContent: "center", padding: "8px 2px", color: active ? "#fff" : "#4A5568", fontWeight: 900, fontSize: 11, whiteSpace: "nowrap", boxShadow: active ? "0 8px 16px rgba(8,105,244,0.16)" : "none", overflow: "hidden" }}>
                  <span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{it.short || it.label}</span>
                </button>
              );
            })}
            <button onClick={function() { setMoreOpen(!moreOpen); }}
              style={{ minWidth: 0, border: "none", borderRadius: 12, background: mobileMoreNav.some(function(it) { return it.key === tab; }) ? "#0869F4" : "transparent", cursor: "pointer", fontFamily: FONT, display: "flex", alignItems: "center", justifyContent: "center", padding: "8px 2px", color: mobileMoreNav.some(function(it) { return it.key === tab; }) ? "#fff" : "#4A5568", fontWeight: 900, fontSize: 15, whiteSpace: "nowrap" }}>···</button>
          </div>
          {moreOpen && (
            <div style={{ position: "absolute", right: 10, top: 92, zIndex: 100, width: 146, background: "#fff", border: "1px solid #E5EAF2", borderRadius: 14, boxShadow: "0 14px 30px rgba(33,64,120,0.18)", padding: 6 }}>
              {mobileMoreNav.map(function(it) {
                return (
                  <button key={it.key} onClick={function() { setTab(it.key); setMoreOpen(false); }}
                    style={{ width: "100%", border: "none", background: tab === it.key ? "#EEF5FF" : "transparent", color: tab === it.key ? BLUE : INK, borderRadius: 10, padding: "10px", textAlign: "left", fontFamily: FONT, fontSize: 12, fontWeight: 900, cursor: "pointer" }}>{it.label}</button>
                );
              })}
            </div>
          )}
        ...
        {navItems.map(function(it) {
          ...
          return (
            <button key={it.key} title={it.label} onClick={function() { setTab(it.key); }}
              style={{ border: "none", borderRadius: 10, background: active ? "#1769F6" : "transparent", cursor: "pointer", fontFamily: FONT, display: "flex", alignItems: "center", justifyContent: sidebarCollapsed ? "center" : "flex-start", gap: sidebarCollapsed ? 0 : 12, padding: sidebarCollapsed ? "14px 0" : "14px 14px", color: "#fff", fontWeight: active ? 900 : 700, fontSize: 15, textAlign: "left", width: "100%", marginBottom: 6, opacity: active ? 1 : 0.82 }}>
      ...
      <div style={{ flex: 1, padding: isMobile ? "12px 8px 24px" : "26px 34px", overflow: "auto", minHeight: isMobile ? "auto" : "100vh", maxWidth: isMobile ? "100%" : 1440, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
...
function AdminDashboard(props) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 15, color: "#42506A", fontWeight: 800, marginBottom: 4 }}>안녕하세요, 관리자님! 👋</div>
      <div style={{ fontSize: isMobile ? 24 : 30, fontWeight: 900, color: "#071C59", marginBottom: 22 }}>오늘도 마스트와 함께해요!</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.2fr 1fr", gap: 16, marginBottom: 20 }}>
        <div style={card({ padding: 0, display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", overflow: "hidden" })}>
          <DashMetric label="전체 학교" value={new Set(members.map(function(m) { return m.school; })).size} unit="개" icon={<NavUser />} color="#0869F4" compact={isMobile} onClick={function() { props.onTab("members"); }} />
        ...
        <div style={card({ position: "relative", minHeight: isMobile ? 214 : 176, overflow: "hidden", padding: isMobile ? 0 : "28px 26px 26px 184px", background: "linear-gradient(135deg,#FFFFFF 0%,#F4F9FF 43%,#EAF4FF 100%)", border: "1px solid rgba(204,221,248,0.8)" })}>
...
function AdminMission(props) {
  ...
  function buildAutoSchoolPick() {
    ...
    var latestMissionDate = assignmentHistory.length ? assignmentHistory[0].mission_date : "";
    var picks = [];
    var skipped = [];
    Object.values(schoolMap).sort(function(a, b) {
      ...
      var pick = null;
      if (sorted.length === 1) {
        if (!last || daysBetweenKST(last.mission_date, today) >= 3) pick = sorted[0];
        else skipped.push(group.school + " 없음(1인)");
      } else if (sorted.length === 2) {
        var first = sorted[0], second = sorted[1];
        if (!last) {
          pick = first;
        } else if (last.member_id === first.id) {
          pick = second;
        } else if (last.member_id === second.id) {
          if (latestMissionDate && last.mission_date === latestMissionDate) {
            skipped.push(group.school + " 없음(2인)");
          } else {
            pick = first;
          }
        } else {
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10, marginBottom: 10 }}>
                <div style={{ border: "1px solid #E5EAF2", background: "#F8FAFF", borderRadius: 12, padding: 12 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#42506A", fontWeight: 700, lineHeight: 1.65 }}>
                    기본은 학교별 하루 1명입니다. 1인 학교는 3일에 한 번, 2인 학교는 가나다순 A → B → 없음 순서로 돌고, 3명 이상 학교는 학교 안에서 가나다순 다음 사람을 배정합니다.
                  </div>
                </div>
              </div>
...
function AdminMissionHistory() {
  ...
  return (
    <div>
      <div style={{ fontSize: isMobile ? 22 : 28, fontWeight: 900, color: "#071C59", marginBottom: 6 }}>과거 미션</div>
      <div style={{ fontSize: 13, color: SUB, fontWeight: 800, marginBottom: 16 }}>등록했던 홍보 미션과 마감 내역을 확인합니다.</div>
    ...
    <div>
      ...
      {loading ? <div style={{ color: SUB }}>불러오는 중...</div> : isMobile ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(function(m) {
            var st = proofStats[m.id] || { total: 0, approved: 0, rejected: 0, rate: 0 };
...
function MemberDetailModal(props) {
  ...
  var load = useCallback(async function() {
    ...
    var stats = {
      ...
      pending: rows.filter(function(r) { return r.status === ST.SUBMITTED || r.status === ST.PENDING; }).length,
      rejected: rows.filter(function(r) { return r.status === ST.REJECTED || r.status === ST.MISSED; }).length,
...
function homeHeroCard(extra) { return Object.assign({ height: 224, background: "#EAF3FF", borderRadius: 24, boxShadow: "0 12px 32px rgba(33,64,120,0.08)", marginBottom: 16, position: "relative", overflow: "hidden" }, extra); }
